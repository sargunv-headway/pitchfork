...
**Product:** Pitchfork is a single Rust binary (daemon supervisor CLI). No external services (databases, containers, etc.) are needed.

**Tooling:** The update script installs `mise` and runs `mise install` which provisions all dev tools defined in `mise.toml` (cargo-nextest, bun, node, usage, hk, etc.). Rust stable is managed via `rustup`. Activate mise in your shell with `eval "$(mise activate bash)"` before running commands, or prefix commands with `mise exec --`.

**Build/lint/test commands:** See the "Build Commands" section above — all commands (`cargo build`, `cargo nextest run`, `cargo fmt --check`, `cargo clippy`) work directly. `mise run ci` orchestrates the full pipeline.

**Flaky e2e tests:** Some timing-sensitive e2e tests (e.g., `test_resource_violation_triggers_retry`, `test_dependency_start_order`) may fail intermittently in containerized/cloud environments. A single flaky e2e failure does not indicate a real regression — re-run with `--no-fail-fast` or retry the specific test to confirm.

**Supervisor state:** Running `pitchfork run` or `pitchfork start` auto-starts the supervisor daemon. State is stored in `~/.local/state/pitchfork/`. If you encounter stale state between runs, use `pitchfork clean` or remove `~/.local/state/pitchfork/`.
